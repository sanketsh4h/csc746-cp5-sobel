...
#pragma omp target data map(to:in[0:nvals], Gx[0:9], Gy[0:9], width, height) map(tofrom:out[0:nvals])
   {
   #pragma omp target teams parallel for
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            // You can call sobel_filtered_pixel here and assign the result to 'out' array
